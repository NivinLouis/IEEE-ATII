                    ...
                    <AnimatePresence custom={direction}>
                        {items.map((item, index) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className="absolute inset-0 w-full h-full overflow-hidden border-[6px] bg-neutral-200 dark:bg-neutral-800 border-white dark:border-neutral-700 shadow-2xl rounded-[2rem]"
                                    initial={{
                                        x: offset * 15,
                                        y: Math.abs(offset) * 6,
                                ...
                                >
                                    {item.image.startsWith("http") || item.image.startsWith("/") ? (
                                        ...
                                        <div className={cn("w-full h-full flex items-center justify-center text-white text-5xl font-black bg-gradient-to-br relative overflow-hidden", item.image)}>
                                            <div className="absolute -top-8 -left-8 w-40 h-40 bg-white/10 rounded-full blur-2xl" />
                                            <div className="absolute -bottom-10 -right-10 w-48 h-48 bg-white/10 rounded-full blur-3xl" />
                                            <span className="relative z-10">{item.title.charAt(0)}</span>
                                        </div>
                                    )}
                                </motion.div>
                            );
